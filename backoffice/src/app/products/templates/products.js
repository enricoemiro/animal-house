...
let Products = {
    render: () => {
        ...
        <div span class="input-group mb-4">
            <input type="text" class="form-control rounded" placeholder="Search for products" id="table-search"
                aria-label="Product search bar">
            <button title="Delete" aria-label="Delete" class="btn btn-outline-danger rounded border-0 mx-2" onclick="return deleteProducts(event)"
            ...
            <button class="btn btn-primary rounded justify-content-end" type="button" data-bs-toggle="modal"
                data-bs-target="#add-item"><i class="fa-solid fa-plus"></i>
                Add Item</button>
            <!-- Modal -->
                            ...
                            <div class="modal-body">
                                <div class="input-group mb-3">
                                    <input type="text" id="name" name="name" class="form-control"
                                        placeholder="Product Name" aria-label="product name" required>
                                </div>
                                <div class="input-group mb-3">
                                    <span class="input-group-text">Price ($)</span>
                                    <input type="number" name="price" id="price" class="form-control" aria-label="Dollar amount"
                                        required>
                                </div>
                                <div class="input-group mb-3">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <input type="file" class="form-control" name="images" id="image" required
                                        multiple />
                                </div>
    ...
    after_render: async () => {
        ...
        document.getElementById('add-product-form').addEventListener("submit", (event) => createProductHandler(event));

        let deleteBtns = document.getElementsByClassName('delete-product-btn')
        for (let btn of deleteBtns) { btn.addEventListener("click", (event) => deleteProductHandler(event)); }

        let editForms = document.getElementsByClassName('edit-product-form')
        for (let form of editForms) { form.addEventListener("submit", (event) => editProductHandler(event)) }
    },
};
