...
let Users2 = {
    ...
    after_render: async () => {
        ...
        document.getElementById('add-user-form').addEventListener("submit", (event) => createUserHandler(event));

        let deleteBtns = document.getElementsByClassName('delete-user-btn')
        for (let btn of deleteBtns) { btn.addEventListener("click", (event) => deleteUserHandler(event)); }

        let editForms = document.getElementsByClassName('edit-user-form')
        for (let form of editForms) { form.addEventListener("submit", (event) => editUserHandler(event)) }
    },
};
