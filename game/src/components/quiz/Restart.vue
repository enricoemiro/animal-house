...
function saveAndQuit() {
  ...
  const st = props.store;
  saveQuizPoints(st.score, 'QUIZ').then((response) => {
...
<template>
  <div class="text-gray-700 w/11-12 mx-auto flex flex-col items-center justify-center h-screen">
    <div class="pb-20 flex flex-col items-center">
      <h1 class="mb-8 text-center text-lg font-medium">
        You got {{ store.rightAnswers }} out of {{ store.questionCount }} questions right.
      </h1>
      <p class="mb-8 text-center text-lg font-medium">Points: {{ store.score }}</p>
      <div class="flex flex-col items-center gap-3">
        ...
        >
          <div v-if="!isLoading" class="flex flex-row items-center justify-center">
            <span class="mr-3">Save and quit</span>
          </div>
          <div v-else>
            <svg
              aria-hidden="true"
